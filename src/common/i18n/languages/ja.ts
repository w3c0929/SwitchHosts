...
const lang: LanguageDict = {
  ...
  all: 'すべて',
  append: '追記',
  as_hosts: 'Hosts コンテンツとして適用',
  as_hosts_desc:
    'オンの場合、取得した内容は hosts コンテンツとして使用されます（システム hosts に書き込み可能）。オフにすると定期取得・トリガー専用になり、内容は hosts の仕組みから除外されます。「ローカル保存パス」でファイルを保存できます。',
  show_content: '右側の内容を表示',
  show_content_desc:
    'オンの場合、この項目を選択すると右側のメインエディタに内容が表示されます。オフにすると内容は非表示になり、リスト内の項目としてのみ存在します。デフォルトはオンです。',
  ...
  no_record: 'なし',
  notify: '通知',
  notify_desc: 'ダウンロードの成功または失敗時に通知を送信します。',
  notify_dingtalk: 'DingTalk',
  notify_dingtalk_secret: '署名シークレット（任意）',
  notify_format: '内容形式',
  notify_format_markdown: 'Markdown',
  notify_format_text: 'テキスト',
  notify_message: '通知内容',
  notify_message_placeholder:
    '空欄でデフォルトのメッセージ。プレースホルダー: {title} {result} {message}',
  notify_other: 'その他',
  notify_wecom: 'WeCom',
  overwrite: '上書き',
  password: 'パスワード',
  ...
  save_path: 'ローカル保存パス',
  save_path_desc:
    '更新のたびに、ダウンロードした内容のコピーもこのファイルパスに保存します。空の場合は保存しません。',
  search: '検索',
  select_all: 'すべて選択',
  selected: '選択済み',
  settings: '設定',
  ...
  system_hosts_history_clear_confirm: 'システムhostsの履歴をすべて削除してもよろしいですか？',
  ...
  url_script_desc:
    'この URL はローカルのスクリプト（.ps1 / .bat / .cmd）を指しています。トリガー型スキームとして動作し、更新時にスクリプトが自動実行され、結果が右側エディタに表示されます。内容がシステム hosts に入ることはありません。スクリプトが生成するファイルは既定でスクリプトと同じフォルダーに出力され、下の「ローカル保存先」で出力先フォルダーを変更できます。',
