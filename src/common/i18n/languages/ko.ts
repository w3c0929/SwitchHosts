...
export default {
  ...
  all: '전체',
  append: '추가',
  as_hosts: '호스트 콘텐츠로 적용',
  as_hosts_desc:
    '켜면 가져온 내용을 호스트 콘텐츠로 사용합니다(시스템 호스트에 기록 가능). 끄면 정기 가져오기/트리거 전용이 되고 내용이 호스트 파이프라인에서 제외되며, 「로컬 저장 경로」로 파일을 저장할 수 있습니다.',
  show_content: '우측 내용 표시',
  show_content_desc:
    '켜면 이 항목을 선택할 때 우측 메인 에디터에 내용이 표시됩니다. 끄면 내용이 숨겨지고 목록의 항목으로만 존재합니다. 기본값은 켜져 있습니다.',
  ...
  no_record: '레코드 없음',
  notify: '알림',
  notify_desc: '다운로드 성공 또는 실패 시 알림을 보냅니다.',
  notify_dingtalk: '딩토크',
  notify_dingtalk_secret: '서명 시크릿 (선택)',
  notify_format: '내용 형식',
  notify_format_markdown: 'Markdown',
  notify_format_text: '텍스트',
  notify_message: '알림 내용',
  notify_message_placeholder:
    '기본 메시지를 사용하려면 비워 두세요. 플레이스홀더: {title} {result} {message}',
  notify_other: '기타',
  notify_wecom: '기업 위챗',
  overwrite: '덮어쓰기',
  password: '패스워드',
  ...
  save_path: '로컬 저장 경로',
  save_path_desc:
    '새로고침할 때마다 다운로드한 내용의 사본도 이 파일 경로에 저장됩니다. 비워 두면 저장하지 않습니다.',
  search: '찾기',
  select_all: '전체 선택',
  selected: '선택',
  settings: '설정',
  ...
  system_hosts_history_clear_confirm: '시스템 호스트 기록을 모두 지우시겠습니까?',
  ...
  url_script_desc:
    '이 URL은 로컬 PowerShell 스크립트(.ps1)를 가리킵니다. 트리거형 스킴으로 동작하여 새로고침 시 스크립트가 자동 실행되고 결과가 오른쪽 편집기에 표시됩니다. 내용이 시스템 hosts에 들어가지 않습니다. 스크립트가 생성한 파일은 기본적으로 스크립트와 같은 폴더에 저장되며, 아래의 "로컬 저장 경로"에서 출력 폴더를 지정할 수 있습니다.',
